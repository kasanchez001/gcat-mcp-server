# Genesys Cloud Org Audit MCP Server v1.9.1 GitHub-Ready Build

This build keeps the Node.js v1.9.0 audit/data-collection toolset and updates the package so it can be stored in a GitHub repository and launched from Claude Desktop using either a local clone or `npx github:owner/repo`.

## Recommended Claude Desktop launch options
...
```json
{
  "mcpServers": {
    "genesys-cloud-data-collector": {
      "command": "node",
      "args": ["C:\\MY_FILES\\AI_Learning\\DEV\\genesys-cloud-org-audit-mcp\\src\\mcp-server.js"],
      "env": {
...
### Direct from public GitHub repo with npx
...
```json
{
  "mcpServers": {
    "genesys-cloud-data-collector": {
      "command": "npx",
      "args": ["-y", "github:YOUR_GITHUB_USERNAME/genesys-cloud-org-audit-mcp"],
      "env": {
        "GC_REGION": "mypurecloud.com",
        "GC_SERVICE_CLIENT_ID": "<client ID>",
        "GC_SERVICE_CLIENT_SECRET": "<client secret>",
...
You can pin a branch/tag/commit with `github:YOUR_GITHUB_USERNAME/genesys-cloud-org-audit-mcp#v1.9.1`.

See `CLAUDE_DESKTOP_CONFIG_EXAMPLES.md` for more examples.

---

# Genesys Cloud Data Collector MCP (Claude Desktop)

Service-mode-only MCP server that provides Genesys Cloud list/export tools to Claude Desktop.

## Required env vars
- `GC_REGION` (e.g. `mypurecloud.com.au`)
- `GC_SERVICE_CLIENT_ID`
- `GC_SERVICE_CLIENT_SECRET`

Optional:
- `API_CONCURRENCY_LIMIT` (default 6)
- `EXPORT_PAGE_SIZE` (default 100)
- `EXPORT_MAX_PAGES` (default 50)
- `SNAPSHOT_ENABLED` (default true)
- `SNAPSHOT_TTL_MS` (default 6h)

## Install & run
```bash
npm i
# set env vars (Windows PowerShell example)
$env:GC_REGION="mypurecloud.com.au"
$env:GC_SERVICE_CLIENT_ID="xxx"
$env:GC_SERVICE_CLIENT_SECRET="yyy"
npm start
```
...
```json
{
  "mcpServers": {
    "genesys-cloud-data-collector": {
      "command": "node",
      "args": ["C:\\Tools\\genesys-cloud-data-collector-mcp\\src\\mcp-server.js"],
      "env": {
        "GC_REGION": "mypurecloud.com.au",
        "GC_SERVICE_CLIENT_ID": "YOUR_CLIENT_ID",
        "GC_SERVICE_CLIENT_SECRET": "YOUR_CLIENT_SECRET",
        "API_CONCURRENCY_LIMIT": "4"
      }
    }
  }
...
# Genesys Cloud Org Audit MCP Upgrade v1.3.0

This build upgrades the server from a data collector into a read-only org audit engine.

## New audit and conversation tools
...
```json
{
  "mcpServers": {
    "genesys-cloud-data-collector": {
      "command": "node",
      "args": [
        "C:\\MY_FILES\\AI_Learning\\DEV\\genesys-cloud-data-collector-mcp\\src\\mcp-server.js"
      ],
      "env": {
        "GC_REGION": "mypurecloud.com.au",
        "GC_SERVICE_CLIENT_ID": "<client ID>",
...
This server is still read-only. It does not change Genesys Cloud configuration.

---

# v1.4.0 MakingChatbots-inspired tool parity upgrade

This upgrade reviewed the MakingChatbots article and public tool docs for `@makingchatbots/genesys-cloud-mcp-server`, then added equivalent/read-only org-audit-friendly tools using this server's `gc_*` naming convention.
